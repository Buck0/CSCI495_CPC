...
// This if statement ensures that the file is opened as expected and gets the entire line in case the filename has a space
	if(fileIn){
...
// This takes in a new filepath by the whole line if there is one or sets fileIn to false which will exit the while loop
		getline(fileIn, fileName);
	}
